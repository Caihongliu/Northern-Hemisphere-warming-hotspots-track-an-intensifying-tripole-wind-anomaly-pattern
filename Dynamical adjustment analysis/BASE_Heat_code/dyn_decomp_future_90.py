...
sys.path.append("/scistor/ivm/rbc843/Paper2/1_dynamic_thermo/BASE_Heat_code/")
...
def preprocess_data(lon, lat, 
    ...
    ##t2m###
    t2m_cut = t2m
    # t2m_cut = t2m.sel(time= (t2m.time.dt.year>= lower_year)&(t2m.time.dt.month.isin([6,7,8])))
    ...
    LOWESS5y_yearly_GMST = yearly_GMST        ## select correct timeperiod["Lowess(5)"][-ny:].to_numpy() 
    GMST_reshaped = np.empty(90*ny,) #empty array with shape of number of days (e.g 90 per summer, per year)
    for i, GMST in enumerate(LOWESS5y_yearly_GMST): ## loop over the GMST data to fill the array
        year_array = np.array(90 * [GMST], dtype=float)
        GMST_reshaped[i*90:(i+1)*90] = year_array[:]
    #create xarray 
    ...
    # stream500_cut = stream500.sel(time=(stream500.time.dt.year>=lower_year)&(stream500.time.dt.month.isin([6,7,8]))) 
    # stream500_cut = stream500_cut.sel(time= stream500_cut.time.dt.year<=upper_year)
    stream500_cut = stream500

    stream500_cut = stream500_cut.sel(lon=slice(lon_bounds[0], lon_bounds[1]), lat=slice(lat_bounds[0], lat_bounds[1]))

    ##########################
    ###  STANDARDIZE DATA  ###
    ##########################
    ## t2m
    t2m_cut=t2m_cut-273.15
    ...
    X_ = X_concated.values
    
    Y_ = t2m_y.values.reshape(X_.shape[0], 1)
...
def decompose_one_gridpoint(Y_, X_, penalty, 
    ...
    T2M_dynamical_std = (stream_weigths_reshaped * stream500_x2).sum(dim=("lon", "lat"))
