...
# make_map_Eurasia(ax9)


def make_composite(composite,lat,lon,ax,title,levels):
    ...
    # clist=['#ffffff','#a60e16']
    # cmap = LinearSegmentedColormap.from_list('cmap',clist)
   
    Colors=('#ffffff','#deebf7','#bce5f8','#8acbef',\
            '#4a91cc','#1f4182')
...
c1=make_composite(network_EE,lat,lon,ax1,'Easterly event related to EE heatwaves',levels=[1000,2000,3000,4000,6000]) 
c2=make_composite(network_CA,lat,lon,ax2,'Easterly event related to NWC heatwaves',levels=[300,500,1000,1500,2000])
c3=make_composite(network_NA,lat,lon,ax3,'Easterly event related to ES heatwaves',levels=[500,1000,1500,2000,3000])
c4=make_composite(network_WNA,lat,lon,ax4,'Easterly event related to WNA heatwaves',levels=[300,500,1000,1500,2000])
...
ax1.text(0,1.15, 'b', fontsize=15.5, fontweight='bold', fontfamily='sans-serif',transform=ax1.transAxes)
ax2.text(0,1.15, 'd', fontsize=15.5, fontweight='bold', fontfamily='sans-serif',transform=ax2.transAxes)
ax3.text(0,1.15, 'f', fontsize=15.5, fontweight='bold', fontfamily='sans-serif',transform=ax3.transAxes)
ax4.text(0,1.15, 'h', fontsize=15.5, fontweight='bold', fontfamily='sans-serif',transform=ax4.transAxes)
# ax5.text(0,1.1, 'j', fontsize=15.5, fontweight='bold', fontfamily='sans-serif',transform=ax5.transAxes)
...
def create_east_europe_rectangle():
    return Rectangle(
        (22, 45),
        38, 20, 
        linewidth=1.5, edgecolor='#fe0000',
        facecolor='none',
...
plt.savefig("/public/home/fcai/abc/2Paper_blocking/Final_figures/Last_sig99_Easterly_network.png", dpi=300, bbox_inches='tight')
plt.show()